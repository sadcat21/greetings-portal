-- إضافة جداول العمل لشركة أروما مع الدوال المطلوبة

-- إنشاء دالة تحديث updated_at أولاً (إذا لم تكن موجودة)
...
CREATE TABLE IF NOT EXISTS public.aroma_employees (
...
-- جدول المناوبات
CREATE TABLE IF NOT EXISTS public.aroma_shifts (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- جدول جدولة العمل
CREATE TABLE IF NOT EXISTS public.aroma_work_schedules (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.aroma_attendance (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  employee_id UUID NOT NULL REFERENCES public.aroma_employees(id) ON DELETE CASCADE,
  work_date DATE NOT NULL,
  clock_in TIME,
...
-- جدول الرواتب
CREATE TABLE IF NOT EXISTS public.aroma_payroll (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  employee_id UUID NOT NULL REFERENCES public.aroma_employees(id) ON DELETE CASCADE,
...
-- جدول الإجازات
CREATE TABLE IF NOT EXISTS public.aroma_leaves (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  employee_id UUID NOT NULL REFERENCES public.aroma_employees(id) ON DELETE CASCADE,
...
-- حذف السياسات الموجودة إذا كانت موجودة وإعادة إنشاؤها
DROP POLICY IF EXISTS "Employees can view own profile" ON public.aroma_employees;
DROP POLICY IF EXISTS "Admins can manage employees" ON public.aroma_employees;
DROP POLICY IF EXISTS "Employees can view shifts" ON public.aroma_shifts;
DROP POLICY IF EXISTS "Admins can manage shifts" ON public.aroma_shifts;
DROP POLICY IF EXISTS "Employees can view schedules" ON public.aroma_work_schedules;
DROP POLICY IF EXISTS "Admins can manage schedules" ON public.aroma_work_schedules;
DROP POLICY IF EXISTS "Employees can view attendance" ON public.aroma_attendance;
DROP POLICY IF EXISTS "Employees can record attendance" ON public.aroma_attendance;
DROP POLICY IF EXISTS "Admins can manage attendance" ON public.aroma_attendance;
DROP POLICY IF EXISTS "Employees can view own payroll" ON public.aroma_payroll;
DROP POLICY IF EXISTS "Admins can manage payroll" ON public.aroma_payroll;
DROP POLICY IF EXISTS "Employees can view leaves" ON public.aroma_leaves;
DROP POLICY IF EXISTS "Employees can request leaves" ON public.aroma_leaves;
DROP POLICY IF EXISTS "Admins can manage leaves" ON public.aroma_leaves;
...
-- إنشاء فهارس لتحسين الأداء (مع التحقق من عدم وجودها)
CREATE INDEX IF NOT EXISTS idx_aroma_employees_code ON public.aroma_employees(employee_code);
CREATE INDEX IF NOT EXISTS idx_aroma_employees_department ON public.aroma_employees(department);
CREATE INDEX IF NOT EXISTS idx_aroma_employees_status ON public.aroma_employees(status);
CREATE INDEX IF NOT EXISTS idx_aroma_work_schedules_employee ON public.aroma_work_schedules(employee_id);
CREATE INDEX IF NOT EXISTS idx_aroma_work_schedules_date ON public.aroma_work_schedules(work_date);
CREATE INDEX IF NOT EXISTS idx_aroma_attendance_employee ON public.aroma_attendance(employee_id);
CREATE INDEX IF NOT EXISTS idx_aroma_attendance_date ON public.aroma_attendance(work_date);
CREATE INDEX IF NOT EXISTS idx_aroma_payroll_employee ON public.aroma_payroll(employee_id);
CREATE INDEX IF NOT EXISTS idx_aroma_payroll_period ON public.aroma_payroll(pay_period_start, pay_period_end);
CREATE INDEX IF NOT EXISTS idx_aroma_leaves_employee ON public.aroma_leaves(employee_id);
CREATE INDEX IF NOT EXISTS idx_aroma_leaves_dates ON public.aroma_leaves(start_date, end_date);
...
-- حذف المحفزات الموجودة إذا كانت موجودة وإعادة إنشاؤها
DROP TRIGGER IF EXISTS update_aroma_employees_updated_at ON public.aroma_employees;
DROP TRIGGER IF EXISTS update_aroma_shifts_updated_at ON public.aroma_shifts;
DROP TRIGGER IF EXISTS update_aroma_work_schedules_updated_at ON public.aroma_work_schedules;
DROP TRIGGER IF EXISTS update_aroma_attendance_updated_at ON public.aroma_attendance;
DROP TRIGGER IF EXISTS update_aroma_payroll_updated_at ON public.aroma_payroll;
DROP TRIGGER IF EXISTS update_aroma_leaves_updated_at ON public.aroma_leaves;
DROP TRIGGER IF EXISTS auto_generate_aroma_employee_code_trigger ON public.aroma_employees;
DROP TRIGGER IF EXISTS calculate_work_hours_trigger ON public.aroma_attendance;
...
-- إدراج بعض المناوبات الافتراضية (فقط إذا لم تكن موجودة)
INSERT INTO public.aroma_shifts (name, start_time, end_time, break_duration) 
SELECT 'الصباحي', '08:00:00', '16:00:00', 30
WHERE NOT EXISTS (SELECT 1 FROM public.aroma_shifts WHERE name = 'الصباحي');

INSERT INTO public.aroma_shifts (name, start_time, end_time, break_duration) 
SELECT 'المسائي', '16:00:00', '00:00:00', 30
WHERE NOT EXISTS (SELECT 1 FROM public.aroma_shifts WHERE name = 'المسائي');

INSERT INTO public.aroma_shifts (name, start_time, end_time, break_duration) 
SELECT 'الليلي', '00:00:00', '08:00:00', 30
WHERE NOT EXISTS (SELECT 1 FROM public.aroma_shifts WHERE name = 'الليلي');

INSERT INTO public.aroma_shifts (name, start_time, end_time, break_duration) 
SELECT 'نصف دوام صباحي', '08:00:00', '12:00:00', 15
WHERE NOT EXISTS (SELECT 1 FROM public.aroma_shifts WHERE name = 'نصف دوام صباحي');

INSERT INTO public.aroma_shifts (name, start_time, end_time, break_duration) 
SELECT 'نصف دوام مسائي', '14:00:00', '18:00:00', 15
WHERE NOT EXISTS (SELECT 1 FROM public.aroma_shifts WHERE name = 'نصف دوام مسائي');