...
const AromaOrderTabsNew: React.FC = () => {
  ...
  const addProductToOrder = (product: AromaProduct, quantity: number = 1) => {
    ...
    if (existingIndex >= 0) {
      // بدل الزيادة دائماً، إذا فُتحنا من زر المنتج (نافذة) نضع الكمية الجديدة مباشرة
      const updatedItems = [...orderItems];
      updatedItems[existingIndex].quantity = updatedItems[existingIndex].quantity + quantity;
  ...
  const handleQuantityConfirm = (quantity: number) => {
    // إذا كنا نعدل عنصراً موجوداً، حدّث الكمية مباشرة
    if (editingItem) {
      const index = orderItems.findIndex(i => i.product_id === editingItem.product_id);
      if (index >= 0) {
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      // اجعل employee_id صالحاً فقط إذا كان موجوداً ضمن جدول الموظفين
      const employeeId = employees.find(e => e.id === orderData.employee_id)?.id || null;
  ...
  const handleEditItem = (item: any) => {
    setEditingItem(item);
    // عين المنتج المحدد لمزامنة صورة وسعر المنتج في النافذة
    const p = products.find(p => p.id === item.product_id) || null;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 rtl">
      ...
      <Tabs defaultValue="new-order" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="customers" className="gap-2">
            ...
            <span className="hidden sm:inline">إضافة تاجر</span>
          </TabsTrigger>
          <TabsTrigger value="orders" className="gap-2">
            <FileText className="w-4 h-4" />
            <span className="hidden sm:inline">قائمة الطلبات ({filteredOrders.length})</span>
          </TabsTrigger>
        </TabsList>
      ...
      <QuantityModal
        ...
        product={editingItem ? products.find(p => p.id === editingItem.product_id) || null : selectedProduct}
        priceType={(selectedCustomer?.default_price_type as 'retail' | 'wholesale' | 'super_wholesale') || 'retail'}
        initialQuantity={editingItem ? editingItem.quantity : (selectedProduct ? getProductQuantity(selectedProduct.id) || 1 : 1)}
